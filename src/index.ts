...
const CustomThemeBodyUpdater =
  webpack.getBySource<Record<string, types.AnyFunction>>("data-client-themes");
...
export function start(): void {
  ...
  inject.after(
    ...
    (
      ...
      if (
        res.clientThemesCSS &&
        res.clientThemesClassName &&
        ClientThemesBackgroundStore!.gradientPreset &&
        ThemeNames![ClientThemesBackgroundStore!.gradientPreset.id]
      ) {
        html.setAttribute(
          "theme-hooker",
          `theme-${ThemeNames![ClientThemesBackgroundStore!.gradientPreset.id]
            .toLowerCase()
            .replaceAll("_", "-")}`,
        );
        body.setAttribute(
